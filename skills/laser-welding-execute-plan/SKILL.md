...
### 第 3 步：高影响假设确认（条件必选）（Step 3: High-Impact Assumption Confirmation）

当以下任一条件成立时，必须触发一次用户确认交互：

- 风险等级 >= Medium
- 假设会影响主指标：节拍、强度、密封、导电、外观
...
- 若用户跳过确认，流程可继续，但最终结论上限为“条件可行（CONDITIONAL GO）”，不得给出“可直接落地（GO）”。
